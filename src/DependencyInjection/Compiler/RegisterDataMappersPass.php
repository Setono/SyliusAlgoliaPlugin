...
final class RegisterDataMappersPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->has('setono_sylius_algolia.data_mapper.composite')) {
            return;
        }

        $dataMapper = $container->getDefinition('setono_sylius_algolia.data_mapper.composite');

        /**
         * @var string $id
         * @var array<array-key, array> $tags
         */
        foreach ($container->findTaggedServiceIds('setono_sylius_algolia.data_mapper') as $id => $tags) {
            foreach ($tags as $tag) {
